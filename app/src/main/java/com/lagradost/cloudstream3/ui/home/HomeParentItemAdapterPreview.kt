...
class HomeParentItemAdapterPreview(
    ...
    clickCallback = { viewModel.click(it) },
    moreInfoClickCallback = { viewModel.popup(it) },
    expandCallback = { viewModel.expand(it) }) {
    ...
    override fun onCreateHeader(parent: ViewGroup): ViewHolderState<Bundle> {
        ...
        if (binding is FragmentHomeHeadTvBinding && isLayout(EMULATOR)) {
            ...
            if (params != null) {
                params.setMargins(params.leftMargin, params.topMargin, marginInPixels, params.bottomMargin)
    ...
    override fun onBindHeader(holder: ViewHolderState<Bundle>) { (holder as? HeaderViewHolder)?.bind() }
    override fun onViewDetachedFromWindow(holder: ViewHolderState<Bundle>) { if (holder is HeaderViewHolder) holder.onViewDetachedFromWindow() }
    override fun onViewAttachedToWindow(holder: ViewHolderState<Bundle>) { if (holder is HeaderViewHolder) holder.onViewAttachedToWindow() }
    ...
    private class HeaderViewHolder(
        ...
        val previewAdapter = HomeScrollAdapter { view, position, item -> viewModel.click(LoadClickCallback(0, view, position, item)) }
            ...
            removeCallback = { v ->
                try {
                    ...
                    AlertDialog.Builder(context).apply {
                        ...
                        setPositiveButton(R.string.delete) { _, _ -> DataStoreHelper.deleteAllResumeStateIds(); viewModel.reloadStored() }
                        show().setDefaultFocus()
                    }
                } catch (t: Throwable) { logError(t) }
            },
            clickCallback = { callback ->
                if (callback.action != SEARCH_ACTION_SHOW_METADATA) { viewModel.click(callback); return@ResumeItemAdapter }
                callback.view.context?.getActivity()?.showOptionSelectStringRes(
                    ...
                    when (actionId + if (isTv) 0 else 1) {
                        ...
                        2 -> if (callback.card is DataStoreHelper.ResumeWatchingResult) { DataStoreHelper.removeLastWatched(callback.card.parentId); viewModel.reloadStored() }
                    }
                }
            })
        private val bookmarkAdapter = HomeChildItemAdapter(
            id = "bookmarkAdapter".hashCode(), nextFocusUp = itemView.nextFocusUpId, nextFocusDown = itemView.nextFocusDownId
        ) { callback ->
            if (callback.action != SEARCH_ACTION_SHOW_METADATA) { viewModel.click(callback); return@HomeChildItemAdapter }
        ...
        fun onSelect(item: LoadResponse, position: Int) {
            ...
            (binding as? FragmentHomeHeadTvBinding)?.apply {
                ...
                scoreText?.let { score ->
                    ...
                    val color = when { rating < 5.0 -> "#eb2f2f".toColorInt(); rating < 8.0 -> "#eda009".toColorInt(); else -> "#3bb33b".toColorInt() }
                ...
                if (!castText.isNullOrBlank()) { homePreviewCast.text = homePreviewCast.context.getString(R.string.cast_format, castText); homePreviewCast.isVisible = true } else homePreviewCast.isVisible = false
        ...
        private val previewCallback = object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                previewAdapter.apply { if (position >= itemCount - 1 && hasMoreItems) { hasMoreItems = false; viewModel.loadMoreHomeScrollResponses() } }
        ...
        init {
            ...
            for ((chip, watch) in toggleList) {
                ...
                chip.setOnCheckedChangeListener { _, isChecked -> if (isChecked) viewModel.loadStoredData(setOf(watch)) else if (toggleList.all { !it.first.isChecked }) viewModel.loadStoredData(emptySet()) }
            }
            headProfilePicCard?.isGone = isLayout(TV or EMULATOR)
            alternateHeadProfilePicCard?.isGone = isLayout(TV or EMULATOR)
            (headProfilePic ?: alternateHeadProfilePic)?.observe(viewModel.currentAccount) { currentAccount -> headProfilePic?.loadImage(currentAccount?.image); alternateHeadProfilePic?.loadImage(currentAccount?.image) }
            ...
            fun showAccountEditBox(context: Context): Boolean {
                ...
                return if (currentAccount != null) {
                    showAccountEditDialog(context = context, account = currentAccount, isNewAccount = false, accountEditCallback = { accountViewModel.handleAccountUpdate(it, context) }, accountDeleteCallback = { accountViewModel.handleAccountDelete(it, context) })
                    true
                } else false
            }
            ...
            (binding as? FragmentHomeHeadTvBinding)?.apply {
                ...
                homePreviewInfoBtt.setOnFocusChangeListener { view, hasFocus -> if (!hasFocus) return@setOnFocusChangeListener; val lastFocusMs = lastFocusTimeoutMs; lastFocusTimeoutMs = System.currentTimeMillis(); if (lastFocusMs + 500L < System.currentTimeMillis()) MainActivity.centerView(view) }
                homePreviewHiddenNextFocus.setOnFocusChangeListener { _, hasFocus -> if (!hasFocus) return@setOnFocusChangeListener; previewViewpager.setCurrentItem(previewViewpager.currentItem + 1, true); homePreviewInfoBtt.requestFocus() }
                homePreviewHiddenPrevFocus.setOnFocusChangeListener { _, hasFocus -> if (!hasFocus) return@setOnFocusChangeListener; if (previewViewpager.currentItem <= 0) (activity as? MainActivity)?.binding?.navRailView?.findViewById<NavigationBarItemView>(R.id.navigation_home)?.requestFocus() else { previewViewpager.setCurrentItem(previewViewpager.currentItem - 1, true); binding.homePreviewInfoBtt.requestFocus() } }
            }
            (binding as? FragmentHomeHeadBinding)?.apply {
                homeSearch.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                    override fun onQueryTextSubmit(query: String): Boolean { viewModel.queryTextSubmit(query); return true }
                    override fun onQueryTextChange(newText: String): Boolean { viewModel.queryTextChange(newText); return true }
                })
            }
        }
        private fun updatePreview(preview: Resource<Pair<Boolean, List<LoadResponse>>>) {
            if (preview is Resource.Success) homeNonePadding.apply { val params = layoutParams; params.height = 0; layoutParams = params } else fixPaddingStatusbarView(homeNonePadding)
            ...
            when (preview) {
                is Resource.Success -> {
                    ...
                    } else previewViewpager.setCurrentItem(0, false)
                }
                else -> {
        ...
        private fun updateResume(resumeWatching: List<SearchResponse>) {
            ...
            if (binding is FragmentHomeHeadBinding || binding is FragmentHomeHeadTvBinding && isLayout(EMULATOR)) {
                ...
                title?.setOnClickListener { viewModel.popup(HomeViewModel.ExpandableHomepageList(HomePageList(title.text.toString(), resumeWatching, false), 1, false), deleteCallback = { viewModel.deleteResumeWatching() }) }
            }
        }
        ...
        fun onViewAttachedToWindow() {
            ...
            previewViewpager.apply {
                ...
                observe(viewModel.availableWatchStatusTypes) { (checked, visible) ->
                    for ((chip, watch) in toggleList) chip.apply { isVisible = visible.contains(watch); isChecked = checked.contains(watch) }
